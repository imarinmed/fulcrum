...
class SystemSnapshot:
    ...
    def __init__(self, settings_dump: Dict[str, Any]):
        ...
        self.fulcrum_version = "0.1.0" # Should be imported from __init__ or similar
...
def ensure_report_dir(out_base: Optional[str], report_date: Optional[str] = None, rtype: Optional[str] = "std") -> str:
...
def _page_from_csv(report_dir: str, name: str, csv_path: str, h2: str, h3: Optional[str] = None) -> str:
...
def build_projects_from_data(report_dir: str, data: Dict[str, Dict]) -> Dict[str, str]:
    ...
    headers_vm = ["project_id","name","machineType","zone","status","creationTimestamp","labels"]
    ...
    for pid, pdata in data.items():
        for inst in pdata.get("instances", []):
            vm_rows.append([
                ...
                ",".join([f"{k}:{v}" for k,v in (inst.get("labels") or {}).items()]),
            ])
    ...
    for pid, pdata in data.items():
        for n in pdata.get("networks", []):
            net_rows.append([pid, n.get("name",""), str(n.get("autoCreateSubnetworks",""))])
    ...
    for pid, pdata in data.items():
        for fw in pdata.get("firewalls", []):
            fw_rows.append([pid, fw.get("name",""), fw.get("network",""), fw.get("direction",""), str(fw.get("priority",""))])
    ...
    if prowler_inputs:
        ...
        headers_p = ["project_id","resource_id","check_id","service","severity","status","framework"]
        rows_p = [[i.get(h, "") for h in headers_p] for i in prowler_items]
        sec_path = os.path.join(report_dir, "projects", "security.md")
        content = header(2, "Security") + table(headers_fw, fw_rows) + header(3, "Prowler Assessments") + table(headers_p, rows_p)
    ...
    for pid, pdata in data.items():
        for b in pdata.get("buckets", []):
            b_rows.append([pid, b.get("name",""), b.get("location",""), b.get("storageClass",""), ",".join([f"{k}:{v}" for k,v in (b.get("labels") or {}).items()])])
    ...
    for pid, pdata in data.items():
        for sql in pdata.get("sql_instances", []):
            ds_rows.append([pid, sql.get("name",""), sql.get("databaseVersion",""), sql.get("region",""), sql.get("gceZone","")])
    ...
    headers_k = ["project_id","name","location","currentMasterVersion","network","subnetwork","labels"]
    ...
    for name, title in [("serverless","Serverless"),("data_analytics","Analytics Services")]:
...
def generate_standard_report(out_base: Optional[str], author: str, report_date: Optional[str] = None, report_version: str = "1.0.0", sa_key_path: Optional[str] = None, prowler_json: Optional[str] = None, prowler_csv: Optional[str] = None, rtype: Optional[str] = "std") -> Dict[str, str]:
    ...
    return {"report_dir": report_dir, "index": index_path, "metadata": meta_path, **pages}
