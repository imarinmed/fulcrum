...
def generate_project_tables(out_dir: str, all_projects: List[str] = None) -> Dict[str, Dict[str, str]]:
    ...
    for pid in sorted(target_projects):
        ...
        index[pid] = {}
        categories = ["compute","storage","networking","serverless","data_analytics","security","kubernetes"]
        for cat in categories:
            rows = cats.get(cat, [])
            headers = ["project_id","resource_name","type","region","creation_date","last_modified","owner","cost_center","tags"]
        ...
        for a in assets:
            if "container.googleapis.com/Cluster" in a.get("assetType", ""):
                ...
                clusters.append({
                    ...
                    "labels": ",".join([f"{k}:{v}" for k,v in (data.get("labels", {}) or {}).items()]),
                })
    return clusters
...
def generate_kubernetes_docs(out_dir: str, author: str) -> Dict[str, str]:
    ...
    headers = ["project_id","name","location","masterVersion","network","subnetwork","labels"]
            ...
            for pid in s.catalog.projects:
                for c in list_gke_clusters(container, pid):
                    clusters.append({
                        ...
                        "labels": ",".join([f"{k}:{v}" for k, v in (c.get("resourceLabels", {}) or {}).items()]),
                    })
        except Exception:
            clusters = []
...
def build_index(out_dir: str, author: str, project_index: Dict[str, Dict[str, str]], kube_index: Dict[str, str]) -> str:
    exec_dir = ensure_exec_dir(out_dir)
    lines: List[str] = []
    ...
    lines.append(f"- [Kubernetes Catalog]({kube_index.get('summary','kubernetes/catalog.md')})")
    ...
    lines.append("- [Executive Report](executive/Head-of-Project-Executive-Report.html)")
...
def generate_asset_summaries(out_dir: str) -> None:
            ...
            for a in assets:
                if predicate(a):
                    ...
                    items.append([
                        ...
                        a.get("createTime", ""),
                        ",".join([f"{k}:{v}" for k,v in (data.get("labels", {}) or {}).items()])
                    ])
        return items
    headers = ["project_id","name","location","created","labels"]
    buckets = collect(lambda a: "storage.googleapis.com/Bucket" in a.get("assetType",""))
    sqls = collect(lambda a: "sqladmin.googleapis.com/Instance" in a.get("assetType",""))
    ...
    write_md(os.path.join(sums_dir, "buckets.md"), "# Buckets\n\n" + md_table(headers, buckets))
    write_md(os.path.join(sums_dir, "sql.md"), "# Cloud SQL\n\n" + md_table(headers, sqls))
    write_md(os.path.join(sums_dir, "vms.md"), "# Virtual Machines\n\n" + md_table(headers, vms))
...
def write_metadata(out_dir: str, author: str, version: str, org_id: str, projects: List[str]) -> None:
    exec_dir = ensure_exec_dir(out_dir)
    meta = {
        "author": author,
        "version": version,
