...
from .core.config import load_yaml, write_yaml, merge_config_projects
from .core.catalog import run_orchestrator, validate_csvs
from .core.logging import setup_logging
from .core.settings import load_settings, save_settings, Settings, get_cli_defaults
from .core.progress import init_projects, read_state
from .core.docs import generate_project_tables, generate_kubernetes_docs, build_index, write_metadata
...
from .prowler.runner import list_checks, run_scan, ProwlerUnavailable
...
def report_generate(
    org_id: Optional[str] = typer.Option(None, "--org-id", "-g", help="Organization ID", show_default=True),
    projects: List[str] = typer.Option([], "--projects", "-p", help="Project IDs", show_default=True),
    out_dir: str = typer.Option("master-report", "--out-dir", "-o", help="Output directory", show_default=True),
    summary_path: Optional[str] = typer.Option(None, help="Destination path for Executive Summary Markdown", show_default=True),
    slides_dir: Optional[str] = typer.Option(None, help="Destination directory for slides", show_default=True),
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Enable detailed logging", show_default=True),
    redact: bool = typer.Option(True, help="Enable stakeholder redaction", show_default=True),
):
    """
    Description:
    ...
    with Progress(
        SpinnerColumn(),
        TextColumn("[bold]{task.fields[project]}[/] {task.fields[phase]} {task.fields[detail]}", justify="left"),
        BarColumn(),
        ...
        for i, pid in enumerate(projects):
            ...
            for ph in ["Resources", "IAM"]:
                project_tasks[(pid, ph)] = progress.add_task(
                    "proj", total=100.0, project=pid, phase=ph, detail="pending", style=color
                )
        env = dict(os.environ)
        env["PROGRESS_PATH"] = progress_path
        env["TMPDIR"] = os.path.join(out_dir, "tmp")
        os.makedirs(env["TMPDIR"], exist_ok=True)
        proc = subprocess.Popen([os.environ.get("PYTHON", sys.executable), os.path.join(out_dir, "scripts", "generate_catalog.py")], env=env)
        while True:
            ...
            for pid, pdata in st.get("projects", {}).items():
                for ph, pstate in pdata.get("phases", {}).items():
                    ...
                    if task_id is not None:
                        progress.update(task_id, completed=pstate.get("progress", 0.0), detail=pstate.get("eta", "--"))
...
def catalog_refresh(out_dir: str = typer.Option("master-report", help="Output directory")):
    """
...
def validate_data(out_dir: str = typer.Option("master-report", help="Output directory")):
    """
...
@app.command(
    "validate-report",
    help=(
        "Validate standardized Markdown report structure, syntax, and links."
    ),
)
def validate_report_cmd(path: str = typer.Option("reports", help="Base path or specific report directory")):
    setup_logging()
    report_dir = path
    if os.path.isdir(path) and not os.path.isfile(os.path.join(path, "index.md")):
        ...
        if candidates:
            ...
            latest = sorted(usable, key=lambda x: x[1])[-1][0] if usable else sorted(candidates)[-1]
...
def dashboard_demo(out_dir: str = typer.Option("master-report", help="Output directory")):
    """
...
def config_set_org_id(org_id: str, config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
    s.org.org_id = org_id
...
def config_add_project(project_id: str, config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
...
def config_remove_project(project_id: str, config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
...
def config_set_timeout(seconds: int, config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
...
def config_set_limit(limit: int, config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
...
def config_set_redaction(enable: bool = typer.Option(True), config: str = typer.Option(None, help="Path to fulcrum.toml")):
    setup_logging()
    s = load_settings(config)
...
def build_executive(
    ...
    exec_out: Optional[str] = typer.Option(None, help="Destination directory for executive report"),
):
    setup_logging()
    env = dict(os.environ)
    if exec_out:
        env["EXEC_OUT"] = exec_out
    cmd = [os.environ.get("PYTHON", sys.executable), os.path.join(out_dir, "scripts", "build_executive_report.py")]
...
@docs_app.command("generate", help=(
...
def docs_generate(
    out_dir: str = typer.Option("master-report", "--out-dir", "-o", help="Catalog output directory", show_default=True),
    author: Optional[str] = typer.Option(None, "--author", "-a", help="Author attribution", show_default=True),
):
    setup_logging()
    s = load_settings(None)
    author = author or s.metadata.author
    with Progress(SpinnerColumn(), TextColumn("[bold]Building documentation[/]"), BarColumn(), TimeElapsedColumn(), console=console) as progress:
        task = progress.add_task("build", total=6)
        project_index = generate_project_tables(out_dir, all_projects=s.catalog.projects)
        from .core.docs import generate_asset_summaries, generate_used_services_summary, generate_kubernetes_csv
        ...
        progress.advance(task)
        write_metadata(out_dir, author, s.metadata.version, s.org.org_id, s.catalog.projects)
        progress.advance(task)
    console.print(f"[green]Documentation generated[/] -> {os.path.join(out_dir,'executive','index.md')}")

reports_app = typer.Typer(help="Generate standardized Markdown reports")
app.add_typer(reports_app, name="reports")

@reports_app.command("standard", help=(
...
def reports_standard(
    out_base: str = typer.Option("reports", "--out-base", "-o", help="Base directory for standardized reports", show_default=True),
    report_date: Optional[str] = typer.Option(None, "--report-date", "-d", help="UTC date in YYYYMMDD", show_default=True),
    author: Optional[str] = typer.Option(None, "--author", "-a", help="Author attribution", show_default=True),
    version: Optional[str] = typer.Option("1.0.0", "--version", "-V", help="Report version", show_default=True),
    sa_key: Optional[str] = typer.Option(None, "--sa-key", "-k", help="Path to service account JSON credentials", show_default=True),
    formats: List[str] = typer.Option(["md","json","csv"], help="Output formats to produce", show_default=True),
    prowler_json: Optional[str] = typer.Option(None, help="Path to Prowler JSON results", show_default=True),
    prowler_csv: Optional[str] = typer.Option(None, help="Path to Prowler CSV results", show_default=True),
):
    """
    Description:
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Collecting GCP data[/]"), BarColumn(), TimeElapsedColumn(), console=console) as progress:
        task = progress.add_task("collect", total=1)
        res = generate_standard_report(out_base, author, report_date, version or s.metadata.version, sa_key, prowler_json, prowler_csv, rtype="std")
        progress.advance(task)
    console.print(f"[green]Standard report generated[/] -> {res['report_dir']}")
    console.print("[cyan]Sections:\n - Compute\n - Networking\n - Security (with optional Prowler)\n - Storage\n - Data Storage (Cloud SQL)\n - Kubernetes (GKE)\n - Index & metadata\n")

@reports_app.command("migrate-names", help=(
...
def reports_migrate_names(
    out_base: str = typer.Option("reports", "--out-base", "-o", help="Base output directory", show_default=True),
):
    setup_logging()
    if not os.path.isdir(out_base):
        console.print(f"[yellow]Base directory not found[/] -> {out_base}")
        raise typer.Exit(code=1)
    entries = [d for d in os.listdir(out_base) if d.startswith("report-") and os.path.isdir(os.path.join(out_base, d))]
    changed = []
    for name in sorted(entries):
        ...
        t = "sec" if (
            os.path.isfile(os.path.join(src, "data", "prowler.csv")) or os.path.isfile(os.path.join(src, "data", "prowler.json"))
        ) else "std"
...
def schedule_reports(cadence: str = typer.Option("weekly", help="Cadence: daily|weekly|monthly"), out_base: str = typer.Option("reports", help="Base output directory")):
    setup_logging()
...
def security_run(
    provider: str = typer.Option("gcp", "--provider", help="Cloud provider", show_default=True),
    projects: List[str] = typer.Option([], "--projects", "-p", help="Project IDs", show_default=True),
    org_id: Optional[str] = typer.Option(None, "--org-id", "-g", help="Organization ID", show_default=True),
    sa_key: Optional[str] = typer.Option(None, "--sa-key", "-k", help="Path to service account JSON", show_default=True),
    out_base: str = typer.Option("reports", "--out-base", "-o", help="Base output directory", show_default=True),
    report_date: Optional[str] = typer.Option(None, "--report-date", "-d", help="UTC date in YYYYMMDD", show_default=True),
    formats: List[str] = typer.Option(["md","json","csv"], "--formats", help="Output formats", show_default=True),
    prowler_bin: Optional[str] = typer.Option(None, "--prowler-bin", help="Path to prowler binary", show_default=True),
    prowler_json: Optional[str] = typer.Option(None, "--prowler-json", help="Path to Prowler JSON output", show_default=True),
    prowler_csv: Optional[str] = typer.Option(None, "--prowler-csv", help="Path to Prowler CSV output", show_default=True),
    api_url: Optional[str] = typer.Option(None, "--api-url", help="Base URL for Prowler App API", show_default=True),
    api_token: Optional[str] = typer.Option(None, "--api-token", help="Token for Prowler App API", show_default=True),
):
    """
    Description:
    ...
    issues = []
    if not projects:
        issues.append("No projects configured. Set catalog.projects in fulcrum.toml or pass --projects.")
    try:
    ...
    if api_url:
        try:
            from .prowler.api import is_api_available, run_scan_api, ProwlerUnavailable as APIUnavailable
            if is_api_available(api_url, api_token):
                with Progress(SpinnerColumn(), TextColumn("[bold]Running Prowler API scan[/]"), BarColumn(), TimeElapsedColumn(), console=console) as progress:
                    task = progress.add_task("api", total=1)
                    res_api = run_scan_api(api_url, api_token, provider, projects, org_id or s.org.org_id)
                    progress.advance(task)
    ...
    if not ran_via_api:
        try:
            ...
            import resource
            start = time.time()
            status = {"completed": 0, "failed": 0, "running": 0, "last": "", "last_level": "", "last_module": "", "errors": 0, "log_size": 0, "log_age": "-", "activity": "Active", "events": deque(maxlen=12)}
            def _to_iso(ts: str) -> str:
                try:
                    if "T" in ts:
                        return ts
                    y = int(ts[0:4]); m = int(ts[5:7]); d = int(ts[8:10]); hh = int(ts[11:13]); mm_ = int(ts[14:16]); ss = int(ts[17:19])
            ...
            def _classify(msg: str, mod: str) -> str:
                ...
                if "instantiating gcp provider" in ml:
                    return "init"
                if "service_disabled" in ml or ("has not been used" in ml and "disabled" in ml):
            ...
            def metrics_table():
                ...
                pct = (status["completed"] / total_checks * 100.0) if total_checks else 0.0
                ...
                if status["last"]:
                    t.add_row("Last Event", f"{status['last_level']} {status['last_module']} :: {status['last']}")
                    guidance = ""
                    # Guidance: enable API link
                    if "has not been used" in status["last"] and "disabled" in status["last"]:
                    ...
                    elif "file_cache is only supported" in status["last"] and status["last_module"] == "__init__":
                ...
                try:
                    ...
                    cpu_time = float(getattr(rnow, "ru_utime", 0.0) + getattr(rnow, "ru_stime", 0.0))
                    wall = now - last_wall if now > last_wall else 0.001
                    used = cpu_time - last_cpu
                    pct = max(0.0, min(100.0, (used / wall) * 100.0)) if wall > 0 else 0.0
                    cpu = f"{pct:0.1f}%"
                    # ru_maxrss typically KB on Unix, bytes on macOS Big Sur+; show MB
                    rss_kb = getattr(rnow, "ru_maxrss", 0)
                    mem_mb = int(rss_kb / 1024) if rss_kb > 1024 else int(rss_kb / (1024*1024))
                    mem = f"{mem_mb} MB"
                t.add_row("CPU", cpu)
                ...
                for e in list(status["events"]):
                    ...
                    if sev in ("ERROR","CRITICAL"):
                        ...
                        style = "cyan"
                    ev.add_row(_to_iso(e.get("ts","")), sev.lower(), e.get("module",""), e.get("type",""), e.get("msg",""), style=style)
                return Group(t, ev)
            def hook(line: str):
                try:
                    import json
                    ...
                    if mod == "__init__" and "file_cache is only supported" in (msg or ""):
                        return
                    status["last"] = msg
                    ...
                    if mchecks:
                        try:
                            ...
                            pass
                    # store event
                    status["events"].append({"ts": obj.get("timestamp",""), "level": (lvl or "INFO").upper(), "module": mod or "", "msg": msg or "", "type": _classify(msg, mod or "")})
                except Exception:
                    text = line.strip()
                    if "file_cache is only supported" in text and "[Module: __init__]" in text:
                        return
                    ...
                    status["events"].append({"ts": ts, "level": status.get("last_level","INFO"), "module": status.get("last_module",""), "msg": status["last"], "type": _classify(status["last"], status.get("last_module",""))})
            ...
            def updater(live):
                ...
                while not stop.is_set():
                    ...
                    rnow = resource.getrusage(resource.RUSAGE_SELF)
                    last_cpu = float(getattr(rnow, "ru_utime", 0.0) + getattr(rnow, "ru_stime", 0.0))
                    last_wall = time.time()
                    try:
                        st = os.stat(log_path)
                    ...
                    try:
                        ...
                        pass
                    live.update(Panel(Align.center(metrics_table()), title="Prowler Scan", border_style="cyan"))
                    time.sleep(0.5)
            with Live(Panel(Align.center(metrics_table()), title="Prowler Scan", border_style="cyan"), console=console, refresh_per_second=30) as live:
                t = threading.Thread(target=updater, args=(live,), daemon=True)
                t.start()
                with prog:
                    res = run_scan(provider, projects, org_id or s.org.org_id, sa_key, data_dir, formats, prowler_bin, progress_hook=hook)
                stop.set(); t.join()
                try:
                    prog.update(task_overall, completed=(total_checks or prog.tasks[task_overall].total))
                except Exception:
                    pass
                if res.get("reason") == "idle_timeout":
                    ...
                    status["last_level"] = "ERROR"
                    status["last_module"] = "fulcrum"
                    status["last"] = f"Scan exited unexpectedly (code={res.get('exit_code')})"
                else:
                    status["activity"] = "Completed"
                    status["last_level"] = "INFO"
                    status["last_module"] = "fulcrum"
                    status["last"] = "Scan finished"
                live.update(Panel(Align.center(metrics_table()), title="Prowler Scan", border_style="green"))
            end_time = time.time()
            ...
            if not os.path.isfile(csv_candidate):
                console.print("[yellow]Prowler CSV not found; ensure a real Prowler CLI is installed and credentials permit scanning.")
                console.print(f"[cyan]Summary[/]: status={final_status.lower()}, duration={duration}s, errors={status['errors']}")
                try:
                    with open(os.path.join(data_dir, "scan_summary.log"), "a") as sf:
                        ts_iso = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(end_time))
                        sf.write(f"time={ts_iso} status={final_status} exit={res['exit_code']} duration={duration}s errors={status['errors']}\n")
                except Exception:
                    pass
            else:
                ...
                try:
                    ...
                    with open(csv_candidate, newline="") as f:
                        ...
                        for row in r:
                            total += 1
                            status_text = (row.get("Status") or row.get("status") or "").upper()
                            if status_text:
                                sc[status_text] = sc.get(status_text, 0) + 1
                            v = (row.get("Severity") or row.get("severity") or "").upper()
                            if v:
                                sev[v] = sev.get(v, 0) + 1
                    p = sc.get("PASS", 0)
                    ...
                    m = sc.get("MANUAL", 0)
                    mu = sc.get("MUTED", 0)
                    sev_str = ", ".join([f"{k.lower()}={sev[k]}" for k in sorted(sev.keys())]) or "none"
                    console.print(f"[cyan]Summary[/]: status={final_status.lower()}, duration={duration}s, checks={total}, pass={p}, fail={fcount}, manual={m}, muted={mu}; severity: {sev_str}")
                    try:
                        with open(os.path.join(data_dir, "scan_summary.log"), "a") as sf:
                            ts_iso = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime(end_time))
                            sf.write(f"time={ts_iso} status={final_status} exit={res['exit_code']} duration={duration}s checks={total} pass={p} fail={fcount} manual={m} muted={mu} severity=[{sev_str}]\n")
                    except Exception:
                        pass
                except Exception:
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Formatting security report[/]"), BarColumn(), TimeElapsedColumn(), console=console) as progress:
        ...
        _ = generate_standard_report(out_base, author, report_date, s.metadata.version, sa_key, prowler_json or None, prowler_csv or prowler_csv_path, rtype="sec")
        progress.advance(task)
...
@security_app.command(
    "ingest",
    help=(
        "Ingest existing Prowler outputs (JSON/CSV) into a standardized report."
    ),
)
def security_ingest(
    out_base: str = typer.Option("reports", "--out-base", "-o", help="Base output directory", show_default=True),
    ...
    s = load_settings(None)
    author = s.metadata.author
    _ = generate_standard_report(out_base, author, report_date, s.metadata.version, None, prowler_json, prowler_csv, rtype="sec")
...
def security_format(
    ...
    s = load_settings(None)
    author = s.metadata.author
    _ = generate_standard_report(out_base, author, report_date, s.metadata.version, sa_key, rtype="sec")
...
def security_validate(path: str = typer.Option("reports", help="Base path or specific report directory")):
...
def migrate_gcr(
    ...
    recursive: bool = typer.Option(False, help="Recursively scan for nested images (e.g. functions)")
):
    setup_logging()
    all_maps = []
    for pid in projects:
...
@app.command("migrate-functions", help="Migrate Cloud Functions (1st gen) images to AR.")
def migrate_functions(
    ...
    for pid in projects:
        ...
        for host_base in ["gcr.io", "eu.gcr.io"]:
            ...
            m = migrate_project(pid, location, dry_run=dry_run, recursive=True, specific_host=gcf_host)
            all_maps.extend(m)
...
def analyze_logging_cmd(
    ...
    for pid in projects:
        ...
        for b in report["buckets"]:
            console.print(f"  - {b['name']} (Retention: {b.get('retentionDays', 'Default')} days)")
...
def decom_audit(
    ...
    with console.status("[bold green]Auditing resources...[/]") as status:
        for pid in projects:
            ...
            console.print(f"  - {pid}: Found {len(report['gke_clusters'])} clusters, {len(report['sql_instances'])} SQLs, {len(report['buckets'])} buckets.")
...
def decom_destroy(
    ...
    if not dry_run:
        confirm = typer.confirm("ARE YOU SURE YOU WANT TO DESTROY RESOURCES? THIS CANNOT BE UNDONE.")
        if not confirm:
            raise typer.Abort()
...
def port_check(
    ...
    all_projects: bool = typer.Option(False, "--all", help="Check all projects in fulcrum.toml"),
    parallel: int = typer.Option(5, help="Max parallel checks"),
    export: Optional[str] = typer.Option(None, "--export", "-e", help="Export format (json)")
):
    ...
    if not projects:
        typer.echo("Please specify projects with --project/-p or use --all to check all projects", err=True)
        raise typer.Exit(1)
...
def audit(
    projects: List[str] = typer.Option([], "--project", "-p", help="GCP Project IDs"),
    all_projects: bool = typer.Option(True, "--all/--select", help="Scan all projects in fulcrum.toml"),
    code_scan: bool = typer.Option(False, "--code", help="Also scan local codebase for secrets"),
    ...
    if not target_projects:
        console.print("[yellow]No projects specified. Use --project or ensure fulcrum.toml has projects.[/]")
        if not code_scan:
            raise typer.Exit(code=1)
    else:
    ...
    if code_scan:
        ...
        if findings:
            console.print(f"[bold red]Found {len(findings)} potential security issues in code:[/]")
            for f in findings:
...
def sec_scan_infra(
    projects: List[str] = typer.Option([], "--project", "-p", help="GCP Project IDs"),
    all_projects: bool = typer.Option(False, "--all", help="Scan all projects in fulcrum.toml"),
    provider: str = typer.Option("gcp", help="Cloud Provider"),
    timeout: int = typer.Option(1200, help="Timeout per project scan (seconds)"),
    parallel: int = typer.Option(3, help="Max parallel scans")
):
...
def _run_infra_scan_tui(target_projects: List[str], provider: str, timeout: int, parallel: int):
    console.print(f"[bold]Starting scan for {len(target_projects)} projects (Max Parallel: {parallel})...[/]")
    ...
    scan_metrics = {pid: {"start": None, "end": None, "counts": {}} for pid in target_projects}
    ...
    def generate_table():
        ...
        sorted_pids = sorted(target_projects, key=lambda p: (scan_status[p] in ["SUCCESS", "FAILED"], p))
            ...
            try:
                ...
                if res.success:
                    ...
                    try:
                        ...
                        pattern = os.path.join("prowler_reports", f"prowler-{pid}*.ocsf.json")
                            ...
                            with open(latest_file, 'r') as jf:
                                try:
                                    ...
                                    for finding in data:
                                        ...
                                        if state_id == 2: status = "PASS"
                                        elif state_id in [0, 1]: status = "FAIL"
                                        
                                        if status == "FAIL":
                                            sev_id = finding.get("severity_id", 0)
                                            sev_name = severity_map.get(sev_id, "UNKNOWN")
    ...
    console.print("[cyan]Aggregated report saved to prowler_reports/aggregated_summary.json[/]")

@security_app.command("report-aggregate")
def sec_report_aggregate(
    report_dir: str = typer.Option("prowler_reports", help="Directory containing Prowler JSON reports")
):
    ...
    console.print(f"Global Criticals: [bold red]{summary['total_stats']['CRITICAL']}[/]")
...
def backup_list(
    projects: List[str] = typer.Option([], "--projects", "-p", help="Project IDs"),
    limit: int = typer.Option(5, "--limit", "-l", help="Max backups to show per cluster")
):
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Fetching backups...[/]"), BarColumn(), console=console) as progress:
        ...
        for pid in projects:
            try:
                ...
                for cluster, group in groupby(backups, key=lambda x: x['cluster']):
                    ...
                    group_list.sort(key=lambda x: x.get('create_time') or "", reverse=True)
                    
                    shown = group_list[:limit]
                    for b in shown:
                        state_style = "green" if b['state'] == "SUCCEEDED" else "red" if b['state'] in ["FAILED", "UNPROTECTED", "NO_BACKUPS"] else "yellow"
                        t.add_row(
                            b['project'], 
                            b['cluster'], 
                            b['plan'], 
...
def backup_inventory(
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Scanning clusters...[/]"), BarColumn(), console=console) as progress:
        task = progress.add_task("scan", total=len(projects))
        for pid in projects:
            try:
                bo = BackupOrchestrator(pid)
...
def backup_protect(
    projects: List[str] = typer.Option([], "--projects", "-p", help="Project IDs"),
    retention_days: int = typer.Option(7, "--retention", "-r", help="Retention period in days")
):
    ...
    for pid in projects:
        console.print(f"[bold]Applying backup protection for {pid} (Retention: {retention_days}d)...[/]")
        try:
            bo = BackupOrchestrator(pid)
            results = bo.protect_unprotected_clusters(retention_days=retention_days)
            for r in results:
                color = "green" if r['status'] == "Created" else "yellow"
                console.print(f"  - {r['cluster']}: [{color}]{r['status']}[/] ({r.get('operation') or r.get('reason') or r.get('error')})")
        except Exception as e:
...
def backup_plan(
    ...
    with Progress(SpinnerColumn(), TextColumn("[bold]Fetching backup plans...[/]"), BarColumn(), console=console) as progress:
        task = progress.add_task("scan", total=len(projects))
        for pid in projects:
            try:
                bo = BackupOrchestrator(pid)
    ...
    console.print(t)
    if export_toml:
        console.print("[green]Exported TOML configurations for projects with active plans.[/]")
...
def auth_sa(sa_key: str = typer.Argument(..., help="Path to service account JSON key")):
    ...
    ok, err = preflight_permission_check(compute, project_id or "")
    if ok:
        console.print(f"[green]Service Account verified[/] (project={project_id}) and saved to {path}")
    else:
        console.print(f"[yellow]Service Account preflight failed[/] -> {err}")

@auth_app.command("impersonate")
def auth_impersonate(service_account_email: str = typer.Argument(..., help="Target service account email")):
    """
    ...
    path = save_settings(None, s)
    console.print(f"[green]Impersonation target saved[/] -> {service_account_email} in {path}")
...
def auth_verify():
    ...
    setup_logging()
    from .gcp.client import build_compute
    from .gcp.auth import load_credentials, load_impersonated_credentials, preflight_permission_check
    s = load_settings(None)
    base_creds, project_id = load_credentials(s.credentials.sa_key_path)
    if s.credentials.impersonate_service_account:
        try:
            base_creds = load_impersonated_credentials(base_creds, s.credentials.impersonate_service_account)
        except Exception as e:
            console.print(f"[yellow]{e}")
