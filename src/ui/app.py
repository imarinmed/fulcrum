...
class Dashboard(App):
    ...
    def on_key(self, event) -> None:
        if getattr(event, "key", "").lower() == "e":
            ...
            subprocess.run([sys.executable, os.path.join(self.out_dir, "scripts", "build_executive_report.py")], env=env)
        ...
        if getattr(event, "key", "").lower() == "d":
            ...
            build_index(self.out_dir, s.metadata.author, {}, {"summary": "kubernetes/catalog.md"})
            write_metadata(self.out_dir, s.metadata.author, s.metadata.version, s.org.org_id, s.catalog.projects)
