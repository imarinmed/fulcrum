...
from .runner import ProwlerUnavailable

log = structlog.get_logger()

class ScanResult:
    def __init__(self, project_id: str, success: bool, report_path: Optional[str] = None, error: Optional[str] = None):
...
class AsyncScanner:
    def __init__(self, output_dir: str = "prowler_reports", timeout_sec: int = 600, max_concurrency: int = 3):
            ...
            try:
                ...
                try:
                    ...
                    if process.returncode not in [0, 3]:
                        ...
                        log.error("scanner.failed", project=project_id, error=err_msg, code=process.returncode)
                        return ScanResult(project_id, False, error=f"Exit {process.returncode}: {err_msg}")
