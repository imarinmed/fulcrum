...
def test_security_ingest_and_validate(tmp_path, monkeypatch):
    ...
    prowler_json.write_text(
        '[{"check_id":"gcp_storage_bucket_public","service":"gcs","status":"FAIL","resource_id":"b1","project_id":"p1","severity":"high"}]'
    )
    ...
    result = runner.invoke(
        app, ["reports", "security", "validate", "--path", str(tmp_path / "reports")]
    )
    assert result.exit_code == 0
